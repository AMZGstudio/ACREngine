...
namespace acre // definitions
{
	void sysDrawShadowedText(int x, int y, Area area, std::string txt, Font font, short front_color, short back_color)
	{
		...
		if (x == Centered) x = areaWidth(area) / 2 - txtWidth(ctxt, font) / 2;
		if (y == Centered) y = areaHeight(area) / 2 - txtHeight(ctxt, font) / 2;
	...
	{
		_window = createArea(areaWidth(Screen), areaHeight(Screen), Default, Default);
	...
	void Menu::drawOption(Option& op, Space _spOptions, const bool hovered)
	{
		...
		spDrawText(op.x, op.y, _spOptions, op.text.c_str(), _font, RGBToColor(op.r, op.g, op.b));
	}

	inline void Menu::optionHovered(Option& op)
	{
		Color r, g, b;
		colorToRGB(Red, &r, &g, &b);
	...
	inline void Menu::calculations(Space optionSpace)
	{
		...
		for (Option& o : _options)
		{
			...
			Space textSpace = _calcSpace(optionSpace, o.x, o.y, txtWidth(o.text.c_str(), _font), txtHeight(o.text.c_str(), _font));
...
#else
#error You need to use ACRE 3.2 and include it before!
#endif