...
#ifdef ACRE_3_2_COMPATIBLE
	#define ACRE_EX_GUI

	#define TEXT_SIZE 100
	#define LIGHT_WINDOW_COL RGBToColor(160, 160, 160)
	#define LIGHT_BUT_ON_COL RGBToColor(100, 100, 100)
	#define LIGHT_BUT_OF_COL RGBToColor(120, 120, 120)
	#define LIGHT_SLIDEB_COL RGBToColor(80, 80, 80)
	#define LIGHT_SLIDEF_COL RGBToColor(100, 100, 100)
	#define LIGHT_TITLE_COL RGBToColor(100, 100, 100)
	#define LIGHT_TEXT_COL Black

	#define DARK_WINDOW_COL RGBToColor(70, 70, 70)
	#define DARK_BUT_ON_COL RGBToColor(80, 80, 80)
	#define DARK_BUT_OF_COL RGBToColor(105, 105, 105)
	#define DARK_SLIDEB_COL RGBToColor(50, 50, 50)
	#define DARK_SLIDEF_COL RGBToColor(100, 100, 100)
	#define DARK_TITLE_COL RGBToColor(100, 100, 100)
...
#undef ACRE_GUI
	...
	bool calculateButton(Option* button)
	{
		if (!pointSpaceOverlap(Mouse.x, Mouse.y, _calcSpace(_windowSpace(button->_window), button->x, button->y, button->width, button->height)))
		{
	...
	bool calculateSlider(Option* slider)
	{
		Space sp = _calcSpace(_windowSpace(slider->_window), slider->x, slider->y, slider->width, slider->height);
	...
	void _drawShade(int xStart, int yStart, int xEnd, int yEnd)
	{
		for (int y = yStart; y < yEnd; y++)
			...
			{
				Color r, g, b;
				colorToRGB(getBackColor(x, y), &r, &g, &b);

				if (y == yStart || x == xStart)
					drawPixel(x, y, RGBToColor(r + 20, g + 20, b + 20));
	...
	void drawOption(Option* op, bool dark)
	{
		...
		case Slider:
		{
			Space s = _calcSpace(spWindow, op->x, op->y, op->width, op->height);
...
#endif
#else
#error You need to use an ACRE 3.2 compatible Version
#endif
