...
Space drawPicture(Space canvas, int xWidth, int yStart, int yWidth)
{
	...
	//for (int x = 1; x < xWidth; x++)
		//for (int y = 1; y < yWidth; y++)
			//sysDrawPoint(x, y, pic, Default, Default,Color(random(0, 255), random(0, 255), random(0, 255)));
...
int main()
{
	...
	while (true)
	{
		...
		Space controlPanel =  drawWindow(canvas.endX - 22, canvas.startY, canvas.endX - 3, canvas.endY - 55, canvas, "Controls");
		...
		drawButton(Centered, 3, 13, 5,  controlPanel, "Play", Grey, DarkGrey);
		drawButton(Centered, 8, 13, 10, controlPanel, "Stop", Grey, DarkGrey);
