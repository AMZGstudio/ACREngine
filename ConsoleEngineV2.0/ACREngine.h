...
int nextOpenSlot = 0, bufferWidth = 100, bufferHeight = 50, fontWidth = 12, fontHeight = 12, defaultFrontColor = White, defaultBackColor = Black;
...
void render(bool clear)
{
		...
		if (ScreenToClient(ConsoleWindow, &cursor_pos));
	{
		...
		Mouse.x = (int)((int)((float)cursor_pos.x) / (float)fontWidth);
		Mouse.y = (int)((int)((float)cursor_pos.y) / (float)fontHeight);
	}
