...
void save(int* screenBuffer, int width, int height)
{
	char* path = "CurrentSave.acre";
	...
	char widthBuf[10] = { 0 };
	_itoa(width, widthBuf, 10);

	char heightBuf[10] = { 0 };
	_itoa(height, heightBuf, 10);

	char bufToWrite[30] = { 0 };
	strcat(bufToWrite, widthBuf);
	strcat(bufToWrite, ";");
	strcat(bufToWrite, heightBuf);
	strcat(bufToWrite, "\n");

	fwrite(bufToWrite, strlen(widthBuf)+strlen(heightBuf)+2, 1, file);
	...
	for (int y = 0; y < height; y++)
	{
		...
		{
			char num[8] = { 0 };
			_itoa(screenBuffer[y * width + x], num, 10);
			strcat(num, ";#");
			fwrite(num, strlen(num), 1, file);
		}
		if(y != height-1)
			fwrite("\n", 1, 1, file);
	}
...
{
	bool darkMode = true;
	initalize("ACRE Painter", 170, 100, 6, 6, Black, darkMode ? Color(30, 30, 30):Color(160,160,160));
...
Changelog:
	...
	Additions:
	- added timePerSec() function. it adds a float value to move something 'amount' of times in a second.
	- added pointCollide() function, it checks if a point collides with a space.
	- added sysDrawColorBuffer() and drawColorBuffer() functions. they draw a 1d buffer used as a 2d buffer, filled with colors.
	- added pointSpaceCollide() function. it checks if a point collides with a space.
	- added strToInt() and intToStr() function.
	- added sysGetPoint(), getPointFront(), and getPointBack()
	- added clamp() function.
	- added Timer struct, and createTimer(), and calculateTimer() functions. 
	- added Alt key.
	- added stringLength() function.

	Fixes:
	- fixed VS warnings
	- fixed bugs in textBoxInput
	- fixed aspect ratio on some computers.
	- fixed fps counter to be more precise.
	- fixed createImage in cpp
	- fixed drawLine() to work with Centered, commands.

	Removed:
	- removed time.h include.
	- removed reduntent COLORVAL struct.
	- removed FPS define macro.

	Miscellaneous:
	- header file now works with multiple includes, across multiple files.
	- 'SHOW_FPS true' is now just SHOW_FPS
	- includes ACRE watermark (can be disabled with NO_ACRE_WATERMARK)
	- renamed createImage to loadImage
	- made error msg's more consistent.
	- added error msg for writing to screenBuffer, with invalid data.
	- created Space convention. startX is the point where the space starts, 
	  and endX is the first point where the point doesnt exist.
*/